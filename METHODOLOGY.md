...
The **feature dictionary** (`contract/feature_dictionary.md`) maps every sub-factor to its measured/disclosed source and exact computation — e.g. `non_firm_intensity` is the MW-weighted share of an asset's capacity on flexible/curtailable connections, read directly from the **DNO Embedded Capacity Register** and the **NESO TEC "Gate" column**, not estimated. After this is applied at scale the Exposure axis is predominantly measured/disclosed, and only three Preparedness sub-factors (`data_monitoring`, `underwriting_expertise`, `pricing_modelling`) remain irreducibly `assessed` — and those are weight-capped (≤ 0.60 of the axis) and reported transparently. **Publication gate: blended measured + disclosed share ≥ 60%** (the mean of the two axes' shares; blended rather than per-axis because the Preparedness axis caps at 40% measurable weight, so a per-axis 60% bar is unsatisfiable there by construction — see `harness/stress_test.py` S2).
